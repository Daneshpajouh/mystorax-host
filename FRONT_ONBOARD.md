...
This installs the local plugin pack, copies skills, and registers the stdio MCP server in `~/.cursor/mcp.json`. The installer stores no token.
...
This copies all `skills/*` entries into `~/.claude/skills`. Register `.mcp.json` through Claude Code's MCP settings or CLI.
...
Use the same skills and MCP server as Claude Code. A separate Conductor contract is not required.
...
This copies all skills into `~/.codex/skills`. Register the stdio MCP server using `.mcp.json`.
